...
public abstract class BSO {
	...
	public static Object[] searchBSO(ClausesSet clset, int flip, int numBees, int maxChances, int numLocalSearch, int maxIterations, boolean withThreads) {
		...
		if(withThreads) { /* "With threads" version choosed : Each bee is a thread */
			...
			while((iteration != maxIterations) && (! sRef.getSolution().isSolution(clset))) {
				...
				if(sRef.getSolution().satisfiedClauses(clset) > bestSolution.satisfiedClauses(clset))
					bestSolution = sRef.getSolution();
				//System.out.println(sRef.getSolution().satisfiedClauses(clset));
				iteration++;
				fitness = bestSolution.satisfiedClauses(clset);
				fitnessList.add(bestSolution.satisfiedClauses(clset));
				time.add(String.valueOf(System.nanoTime()/ 1000000000));

				dance.clear();		bees.clear(); /* Clear "dance" and "bees" tables for the next iteration */
			}
		}

		Object[] obj =new Object[]{bestSolution, fitnessList, time, iteration, fitness};
